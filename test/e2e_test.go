...
func newTestEnv(t *testing.T) *testEnv {
	...
	return &testEnv{t: t, rootDir: rootDir, dataDir: dataDir, repo: repo}
}
...
func (e *testEnv) createSession(dir string) {
	e.t.Helper()
	cmd := exec.Command("opencode", "run", "respond with the single word OK", "--dir", dir)
	cmd.Env = append(os.Environ(), "XDG_DATA_HOME="+e.dataDir)
	out, err := cmd.CombinedOutput()
	if err != nil {
		e.t.Fatalf("opencode run failed in %s: %v\n%s", dir, err, out)
	}
}
...
func TestTargetedRm_ForceOverrides(t *testing.T) {
	...
	assertContains(t, out, "Removed force (forced)")
...
func TestTargetedRm_CleanNoSession(t *testing.T) {
	...
	out := env.wt("rm", "clean")
	assertContains(t, out, "Removed clean")
...
func TestTargetedRm_MergedBranch(t *testing.T) {
	...
	assertContains(t, out, "Removed merged")
...
func TestTargetedRm_PushedNotMergedWarns(t *testing.T) {
	...
	assertContains(t, out, "Removed in-review")
